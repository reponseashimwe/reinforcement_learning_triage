"""
DQN Training Script
Train Deep Q-Network agent on ClinicEnv with best configuration.
...
def train_dqn(
    ...
    model_path = os.path.join(output_dir, f"dqn_seed{seed}.zip")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train DQN agent on ClinicEnv")
    ...
    parser.add_argument("--output_dir", type=str, default="models/dqn", help="Output directory for models")
    parser.add_argument("--verbose", type=int, default=1, help="Verbosity level (0=quiet, 1=info)")
    args = parser.parse_args()

    best_config = {
        "id": "dqn_fast_target_update",
        "description": "More frequent target network updates",
    ...
    print(f"\n{'=' * 80}")
    print("DQN TRAINING - BEST CONFIGURATION")
    print(f"{'=' * 80}")
